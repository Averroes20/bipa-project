...
class PronunciationService:
    ...
    def detect_errors(target_text: str, transcribed_words: List[Dict], phonemes_data: List[Dict] = None) -> Dict[str, List[Dict]]:
        ...
        return {"word_errors": word_errors, "phoneme_errors": phoneme_errors}
...
class ScoringService:
    ...
    def calculate_scores(
        ...
        """
        Calculates all required scores with metadata.
        """
        ...
        # Intonation Score (based on Pitch stats and Native similarity)
        pitch_variance = pitch_stats.get("variance", 0)
        intonation_score = max(0, min(100, ac_sim * 0.6 + min(40, pitch_variance / 50)))
